...
void setup() {
  ...
  // Set up AP mode
  const char* apSSID = "FloodMonitorAP";
  const char* apPassword = "monitor123"; // Password must be at least 8 characters
  ...
  // Play WiFi success sound (even though it's AP mode, it's still a successful network setup)
...
server.on("/sensor_status", handleSensorStatus);
server.on("/toggle_sleep", handleToggleSleepMode);
server.on("/sleep_status", handleSleepStatus);

// Add this new route for chart.js
server.on("/chart.js", []() {
